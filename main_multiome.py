...
class Trainer:
    ...
    def generate(self, num_samples, cell_type_condition=None, context_embedding=None):
        ...
        z_gen = self.diff.sample(z_shape, cell_type_labels=gen_labels, context_embedding=context_embedding)
...
if __name__ == '__main__':
    ...
    try:
        ...
        train_dataset = LaplaceDataset(h5ad_path=TRAIN_H5AD, k_neighbors=K_NEIGHBORS, pe_dim=PE_DIM, root=TRAIN_DATA_ROOT, train=True, gene_threshold=GENE_THRESHOLD, pca_neighbors=PCA_NEIGHBORS, seed=GLOBAL_SEED, dataset_name='multiome')
    ...
    if num_train_cells > 0 and input_feature_dim > 0:
        ...
        for epoch in epoch_bar:
            (avg_total_loss, avg_diff_loss, avg_kl_loss,
             ...
             avg_gene_stats, avg_zero_rate) = trainer.train_epoch(train_loader, epoch)
    ...
    if not filtered_gene_names_from_train:
        ...
        try:
            test_dataset = LaplaceDataset(h5ad_path=TRAIN_H5AD, k_neighbors=K_NEIGHBORS, pe_dim=PE_DIM, root=TEST_DATA_ROOT, train=False, gene_threshold=GENE_THRESHOLD, pca_neighbors=PCA_NEIGHBORS, seed=GLOBAL_SEED, dataset_name='multiome')
