...
def _resolve_counts(a):
    if hasattr(a, "layers") and "counts" in a.layers:
...
def _ensure_cell_type(adata, label_key):
    if label_key in adata.obs.columns:
        return adata
    return adata


def _maybe_leiden(adata, label_key="cell_type"):
    if label_key in adata.obs.columns:
        return adata
    adata = adata.copy()
    ...
    sc.tl.leiden(adata, key_added=label_key, resolution=0.5)
    return adata


def _sample_counts(model, n_samples):
    ...
    if hasattr(model, "get_normalized_expression"):
        norm = model.get_normalized_expression(n_samples=n_samples, return_numpy=True)
        return norm
    raise RuntimeError("Model has no sampling method.")
...
def evaluate_generation(real_adata, gen_counts, gen_cell_types, n_pcs=30):
...
def evaluate_tstr(real_adata, gen_counts, gen_cell_types):
    from sklearn.ensemble import RandomForestClassifier
    from sklearn.metrics import accuracy_score, f1_score
    if gen_cell_types is None or "cell_type" not in real_adata.obs:
        return {}
    X_syn = _normalize_log1p(gen_counts)
    y_syn = gen_cell_types.astype(int)
...
def main():
    ...
    parser.add_argument("--h5ad", default="data/raw/visium_lymph_node.h5ad")
    parser.add_argument("--seed", type=int, default=77)
    parser.add_argument("--train_frac", type=float, default=0.8)
    parser.add_argument("--label_key", type=str, default="cell_type")
    parser.add_argument("--scvi_epochs", type=int, default=200)
    ...
    counts, var_names, counts_source = _resolve_counts(adata_raw)
    ...
    train_idx, test_idx = _split_indices(n_cells, args.train_frac, args.seed)
    adata_train = adata[train_idx].copy()
    adata_test = adata[test_idx].copy()

    if args.label_key not in adata_train.obs.columns:
        adata_train = _maybe_leiden(adata_train, label_key=args.label_key)
    if args.label_key not in adata_test.obs.columns:
        adata_test.obs[args.label_key] = "Unknown"

    from scvi.model import SCVI, SCANVI
    batch_key = "batch" if "batch" in adata_train.obs.columns else None
    SCVI.setup_anndata(adata_train, batch_key=batch_key)
    scvi_model = SCVI(adata_train)
    scvi_model.train(max_epochs=args.scvi_epochs, batch_size=args.batch_size)

    scvi_gen = _sample_counts(scvi_model, adata_test.shape[0])
    scvi_gen = scvi_gen.toarray() if sp.issparse(scvi_gen) else np.asarray(scvi_gen)

    train_cell_types = (
        adata_train.obs[args.label_key].astype("category").cat.categories.tolist()
        if args.label_key in adata_train.obs
        else ["Unknown"]
    )
    if args.label_key in adata_train.obs:
        train_codes = adata_train.obs[args.label_key].astype("category").cat.codes.values
        probs = np.bincount(train_codes) / np.sum(np.bincount(train_codes))
        gen_labels = np.random.choice(len(probs), size=adata_test.shape[0], p=probs)
    else:
        gen_labels = np.zeros(adata_test.shape[0], dtype=int)

    out_scvi = _experiment_dir(args.output_base, "baseline_spatial_scvi", vars(args))
    generate_qualitative_plots(
        real_adata_filtered=adata_test,
        generated_counts=scvi_gen,
        generated_cell_types=gen_labels,
        train_cell_type_categories=train_cell_types,
        train_filtered_gene_names=adata_train.var_names.tolist(),
        output_dir=out_scvi,
        model_name="scVI",
    )
    plot_zero_rate_calibration(adata_test, scvi_gen, output_dir=out_scvi, model_name="scVI")

    metrics = evaluate_generation(adata_test, scvi_gen, gen_labels)
    metrics.update(evaluate_tstr(adata_test, scvi_gen, gen_labels))
    metrics.update(evaluate_grn(adata_test, scvi_gen))
    metrics.update(evaluate_marker_genes(adata_test, scvi_gen, gen_labels))
    metrics.update(evaluate_infill(adata_test))
    metrics.update(evaluate_interpolation(adata_test))
    with open(os.path.join(out_scvi, "metrics.json"), "w", encoding="utf-8") as f:
        json.dump(metrics, f, indent=2)

    # scANVI
    scvi_model.save(os.path.join(out_scvi, "scvi_model"), overwrite=True)
    scanvi_model = SCANVI.from_scvi_model(scvi_model, adata_train, labels_key=args.label_key, unlabeled_category="Unknown")
    scanvi_model.train(max_epochs=args.scanvi_epochs, batch_size=args.batch_size)

    scanvi_gen = _sample_counts(scanvi_model, adata_test.shape[0])
    scanvi_gen = scanvi_gen.toarray() if sp.issparse(scanvi_gen) else np.asarray(scanvi_gen)
    try:
        gen_adata = AnnData(X=scanvi_gen)
        gen_adata.var_names = adata_train.var_names
        gen_pred = scanvi_model.predict(gen_adata)
        if isinstance(gen_pred, np.ndarray):
            gen_labels_scanvi = gen_pred
        else:
            gen_labels_scanvi = np.asarray(gen_pred)
        if not np.issubdtype(gen_labels_scanvi.dtype, np.integer):
            gen_labels_scanvi = sc.AnnData(obs={"ct": gen_labels_scanvi}).obs["ct"].astype("category").cat.codes.values
    except Exception:
        gen_labels_scanvi = gen_labels

    out_scanvi = _experiment_dir(args.output_base, "baseline_spatial_scanvi", vars(args))
    generate_qualitative_plots(
        real_adata_filtered=adata_test,
        generated_counts=scanvi_gen,
        generated_cell_types=gen_labels_scanvi,
        train_cell_type_categories=train_cell_types,
        train_filtered_gene_names=adata_train.var_names.tolist(),
        output_dir=out_scanvi,
        model_name="scANVI",
    )
    plot_zero_rate_calibration(adata_test, scanvi_gen, output_dir=out_scanvi, model_name="scANVI")

    metrics = evaluate_generation(adata_test, scanvi_gen, gen_labels_scanvi)
    metrics.update(evaluate_tstr(adata_test, scanvi_gen, gen_labels_scanvi))
    metrics.update(evaluate_grn(adata_test, scanvi_gen))
    metrics.update(evaluate_marker_genes(adata_test, scanvi_gen, gen_labels_scanvi))
    metrics.update(evaluate_infill(adata_test))
    metrics.update(evaluate_interpolation(adata_test))
    with open(os.path.join(out_scanvi, "metrics.json"), "w", encoding="utf-8") as f:
        json.dump(metrics, f, indent=2)
