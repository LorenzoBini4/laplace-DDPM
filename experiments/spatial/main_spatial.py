...
class Trainer:
    ...
    def evaluate_marker_genes(self, real_adata, generated_counts, generated_cell_types, top_k=5):
        ...
        try:
            ...
            real_adata_raw.obs['cell_type'] = real_adata_raw.obs['cell_type'].astype(str)
            ...
            gen_adata.obs['cell_type'] = gen_adata.obs['cell_type'].astype(str)
...
if __name__ == '__main__':
    ...
    # PATHS FOR SPATIAL
    TRAIN_H5AD = 'data/raw/visium_lymph_node.h5ad'
    TEST_H5AD = 'data/raw/visium_lymph_node.h5ad' 
    DATA_ROOT = 'data/processed/spatial'
    ...
    if test_adata is not None and test_adata.shape[0] > 0:
        ...
        if 'spatial' in test_adata.obsm:
            morans = trainer.compute_morans_i(test_adata)
            spec = trainer.compute_spectral_mismatch(test_adata)
            eval_log = {}
            eval_log.update(_flatten_metrics("eval/morans_i", morans))
            eval_log.update(_flatten_metrics("eval/spectral_mismatch", spec))
            _wandb_log(wandb_run, eval_log, step=last_epoch)
        else:
        ...
        if generated_datasets_counts and VIZ:
             ...
             _wandb_log_image_dir(wandb_run, output_plot_dir, step=last_epoch, prefix="plots/spatial")
